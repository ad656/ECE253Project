...
def non_local_dehazing(img_hazy, air_light, gamma=1.0,
    ...
    tess_file = f"{tessellation_prefix}{n_points}.txt"
    points = np.loadtxt(tess_file)  # shape (n_points, 3)

    # KD-tree search for nearest tessellation point for each pixel
    tree = cKDTree(points)
    dist, ind = tree.query(dist_unit_radius)   # ind: (h*w,), 0-based indices

    # --- Estimating Initial Transmission ---

    # K: maximal radius per haze-line (Eq. (11)), accumarray(@max)
    radius_flat = radius.ravel()
    K = np.zeros(n_points, dtype=np.float64)
    np.maximum.at(K, ind, radius_flat)   # in-place max per index

    # radius_new(y,x) = K[ind(y,x)]
    radius_new = K[ind].reshape(h, w)

    # transmission = radius / radius_new (Eq. (12))
    eps = 1e-12
    ...
    # trans_lower_bound = 1 - min( img/air_light, over channels )
    ...
    # Reliability via bin counts and std

    # bin_count: number of pixels per haze-line
    ...
    # K_std: std(radius) per haze-line (accumarray @std)
    # We’ll compute this via sums and sums of squares.
    ...
    var_r[var_r < 0] = 0  # numerical safety

    K_std = np.sqrt(var_r)  # population-like std
    ...
    leave_haze = 1.06  # leave a bit of haze for a natural look
